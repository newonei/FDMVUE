        ...
        lines: [
          {
            ...
            allocationEvidenceSourceSystem: 'FDM_WAREHOUSE',
...
describe('mES factory supply task API contract', () => {
  ...
  it('normalizes every business Long ID to string in page and detail', async () => {
    ...
    const page = await getSupplyTaskPage({ pageNo: 1, pageSize: 20 });
    const detail = await getSupplyTask('9');

    expect(requestMocks.get).toHaveBeenNthCalledWith(
      1,
      '/fdmfactory/supply-task/page',
      { params: { pageNo: 1, pageSize: 20 } },
    );
    expect(requestMocks.get).toHaveBeenNthCalledWith(
      2,
      '/fdmfactory/supply-task/get',
      { params: { id: '9' } },
    );
  ...
  it('uses the exact route-scoped model and generic document-generation endpoints', async () => {
    ...
    const models = await searchSupplyTaskAiModels();
    ...
    const run = await getSupplyTaskGeneration('22');
    await retrySupplyTaskGeneration('22', 3);
    await regenerateSupplyTaskGeneration('22', {
      expectedVersion: 4,
    ...
    expect(requestMocks.post).toHaveBeenNthCalledWith(
      ...
      expect.objectContaining({ generationType: 'DEMAND_TO_SUPPLY_TASK' }),
    );
  ...
  it('posts only editable intent when materializing a READY proposal', async () => {
    ...
    expect(requestMocks.post).toHaveBeenCalledWith(
      '/fdmfactory/supply-task/materialize-from-generation',
  ...
  it('rejects unsafe numeric identities instead of losing precision', async () => {
    ...
    await expect(getSupplyTaskGeneration('22')).rejects.toThrow(
      'generation.runId',
    );
  });
