import type { FdmProcurementPurchaseOrderApi } from './index';
...
import {
  ...
  normalizePurchaseOrderProjectionState,
...
function rawPurchaseOrderProjection() {
  return {
    approvalSnapshotHash: 'a'.repeat(64),
    approvalSnapshotId: 601,
    attemptCount: 1,
    companyId: 1,
    commandId: 'PROC_PO_700_101_USD',
    cancelReason: null,
    lastAction: 'CREATE',
    lastActorUserId: 164,
    lifecycleVersion: 0,
    payloadHash: 'b'.repeat(64),
    purchaseOrderId: 9901,
    purchaseOrderNo: 'CG-9901',
    purchaseOrderStatus: 'DRAFT',
    statusUpdatedAt: '2026-08-29T18:30:00',
    exchangeRateToCny: '7.100000000000',
    id: 801,
    lines: [
      {
        cnyBaseUnitPrice: '61.94690264',
        writeModelProductId: 3001,
    ...
    events: [
      {
        ...
        documentVersion: 1,
        eventId: 'fdm-purchase-receipt:9001:v1:post',
      ...
      },
      {
        ...
        actorUserId: null,
        documentVersion: 2,
        eventId: 'fdm-purchase-receipt:9001:v2:reverse',
        fromPostingState: 'POSTED',
        occurredAt: '2026-08-29T21:00:00',
        postingState: 'DRAFT',
        reason: '质检退回',
        result: 'APPLIED',
        reversesEventId: 'fdm-purchase-receipt:9001:v1:post',
      },
    ],
    lastAction: 'REVERSE',
    lastActorUserId: 164,
    lastEventId: 'fdm-purchase-receipt:9001:v2:reverse',
...
describe('procurement purchase-order purchaseOrderProjection API', () => {
  beforeEach(() => vi.clearAllMocks());

  it('normalizes every backend Long before exposing the ledger to components', async () => {
    requestMocks.get.mockResolvedValueOnce([rawPurchaseOrderProjection()]);

    const result = await getPurchaseOrderProjections('88');

    expect(requestMocks.get).toHaveBeenCalledWith(
      '/fdmprocurement/purchase-order/list',
    ...
    expect(typeof result[0]?.lastActorUserId).toBe('string');
  ...
  it('uses dedicated get and optimistic manual-retry endpoints', async () => {
    requestMocks.get.mockResolvedValueOnce(rawPurchaseOrderProjection());
    requestMocks.post.mockResolvedValueOnce(rawPurchaseOrderProjection());

    await getPurchaseOrderProjection('801');
    await retryPurchaseOrderProjection({ expectedVersion: 2, id: '801' });

    expect(requestMocks.get).toHaveBeenCalledWith(
      '/fdmprocurement/purchase-order/get',
      { params: { id: '801' } },
    );
    expect(requestMocks.post).toHaveBeenCalledWith(
      '/fdmprocurement/purchase-order/retry',
  ...
  it('loads projection state and normalizes requisition/outbox Long values', async () => {
    ...
    const result = await getPurchaseOrderProjectionState('88');

    expect(requestMocks.get).toHaveBeenCalledWith(
      '/fdmprocurement/purchase-order/projection-state',
      { params: { requisitionId: '88' } },
    );
  ...
  it('requeues only the explicit projection identity and normalizes the returned state', async () => {
    ...
    const result = await retryPurchaseOrderPublication({
      expectedVersion: 9,
      outboxId: '700',
      reason: '已确认上游映射修复',
      requisitionId: '88',
    ...
    expect(requestMocks.post).toHaveBeenCalledWith(
      '/fdmprocurement/purchase-order/retry-projection',
      {
        expectedVersion: 9,
        outboxId: '700',
        reason: '已确认上游映射修复',
  ...
  it('loads lifecycle history and preserves backend ordering while normalizing actor Long values', async () => {
    requestMocks.get.mockResolvedValueOnce([
      {
        action: 'CONFIRM',
        actorUserId: '9007199254740993',
        scopeKey: 'tenant:1:company:1',
        eventId: 'fdm-purchase-order:9901:v1',
        fromStatus: 'DRAFT',
        lifecycleVersion: 1,
      ...
      },
      {
        ...
        actorUserId: null,
        scopeKey: 'tenant:1:company:1',
        eventId: 'fdm-purchase-order:9901:v2',
    ...
    const result = await getPurchaseOrderLifecycleEvents('801');

    expect(requestMocks.get).toHaveBeenCalledWith(
      '/fdmprocurement/purchase-order/lifecycle-events',
      { params: { projectionId: '801' } },
    );
  ...
  it('normalizes an actor id without changing lifecycle evidence fields', () => {
    expect(
      normalizePurchaseOrderLifecycleEvent({
        ...
        scopeKey: 'tenant:1:company:1',
        eventId: 'fdm-purchase-order:9901:v3',
        fromStatus: 'DRAFT',
        lifecycleVersion: 3,
        occurredAt: '2026-08-29T21:00:00',
        processedAt: null,
  ...
  it('lazily loads execution documents and normalizes every browser-facing Long', async () => {
    ...
    const result = await getPurchaseOrderExecutionFacts('801');

    expect(requestMocks.get).toHaveBeenCalledWith(
      '/fdmprocurement/purchase-order/execution-facts',
      { params: { projectionId: '801' } },
    );
    expect(result[0]).toMatchObject({
      documentId: '9223372036854775807',
  ...
  it('normalizes one execution document without altering quantities or immutable event evidence', () => {
    const normalized = normalizePurchaseOrderExecutionDocument(
      rawExecutionDocument() as unknown as FdmProcurementPurchaseOrderApi.ExecutionDocument,
    );
    ...
    expect(normalized.events[1]?.reversesEventId).toBe(
      'fdm-purchase-receipt:9001:v1:post',
    );
  ...
  it('normalizes return-line identity and preserves authoritative net receipt totals', () => {
    const normalized = normalizePurchaseOrderExecutionDocument(
      rawReturnExecutionDocument() as unknown as FdmProcurementPurchaseOrderApi.ExecutionDocument,
    );
