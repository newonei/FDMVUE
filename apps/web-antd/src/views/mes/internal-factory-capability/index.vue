...
} from '#/api/mes/internal-factory-capability';
...
defineOptions({ name: 'MesInternalFactoryCapability' });

type Capability = MesInternalFactoryCapabilityApi.Capability;
type CapabilityStatus = MesInternalFactoryCapabilityApi.CapabilityStatus;
...
const canQuery = computed(() =>
  hasAccessByCodes(['mes:internal-factory-capability:query']),
);
const canCreate = computed(() =>
  hasAccessByCodes(['mes:internal-factory-capability:create']),
);
const canUpdate = computed(() =>
  hasAccessByCodes(['mes:internal-factory-capability:update']),
);
const canQueryFactories = computed(() =>
  hasAccessByCodes(['mes:internal-factory:query']),
);
...
const factoryOptions = ref<
  Array<{
    factory: MesInternalFactoryCapabilityApi.InternalFactory;
    label: string;
    value: string;
  }>
>([]);
...
const evidenceModeOptions = [
  ...
  label: string;
  value: MesInternalFactoryCapabilityApi.EvidenceMode;
}>;
...
async function loadFactories() {
  ...
  try {
    const result = await getInternalFactoryList({ companyId: companyId.value });
    factoryOptions.value = result.map((factory) => ({
      factory,
      label: `${factory.factoryCode} · ${factory.factoryName} · ID ${factory.id}`,
      value: factory.id,
    }));
    factoryOptionsCompanyId.value = companyId.value;
  } catch {
...
async function openEdit(row: Capability) {
  ...
  try {
    const current = await getInternalFactoryCapability({
...
async function save() {
  ...
  try {
    if (editing.value) {
      await updateInternalFactoryCapability({
        ...result.data,
        id: editing.value.id,
      ...
      const id = await createInternalFactoryCapability(result.data);
...
<template>
  ...
  >
    ...
    <Alert
      v-if="!canQuery"
      description="缺少 mes:internal-factory-capability:query 权限，页面不会读取任何能力数据。"
      message="无权查看工厂产品能力"
      show-icon
      type="error"
    />
    ...
    <div v-else class="capability-page">
      ...
      <Card size="small" title="查询条件">
        ...
        <p class="capability-page__hint">
          工厂下拉需要 mes:internal-factory:query；无该权限时仍可直接输入工厂
          ID。
        </p>
      </Card>
          ...
          <template #bodyCell="{ column, record }">
            <template v-if="column.key === 'identity'">
              <div class="capability-page__stack">
                ...
                <span>产品 SKU ID {{ record.productSkuId }}</span>
      ...
      <div class="capability-form">
        ...
        <section>
          ...
          <div class="capability-form__grid">
            ...
            <label>
              产品 SKU ID
              <Input v-model:value="form.productSkuId" placeholder="正整数 ID">
