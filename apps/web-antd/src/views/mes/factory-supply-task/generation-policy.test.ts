...
import {
  ...
  FACTORY_TASK_AI_PERMISSION_CODES,
...
function detail(): MesFactorySupplyTaskApi.GenerationDetail {
  return {
    attempts: [],
    companyId: '1',
    currentAttemptNo: 1,
...
describe('factory task generation policy', () => {
  it('requires confirmed positive internal-factory demand and every permission', () => {
    expect(canGenerateFactorySupplyTask(plan(), () => true)).toBe(true);
    expect(canGenerateFactorySupplyTask(plan('DRAFT'), () => true)).toBe(false);
    expect(
      canGenerateFactorySupplyTask(plan('CONFIRMED', '0.000'), () => true),
    ).toBe(false);
    expect(
      canGenerateFactorySupplyTask(plan('CONFIRMED', null), () => true),
    ).toBe(false);
    for (const denied of FACTORY_TASK_AI_PERMISSION_CODES) {
      expect(
        canGenerateFactorySupplyTask(plan(), (code) => code !== denied),
      ).toBe(false);
    }
  });

  it('parses the normalized READY proposal and rejects malformed tokens', () => {
    expect(parseFactoryTaskProposal(normalizedJson())).toMatchObject({
      lineSelections: [
    ...
    expect(
      parseFactoryTaskProposal(normalizedJson('factory-1')),
    ).toBeUndefined();
    expect(parseFactoryTaskProposal('{broken')).toBeUndefined();
  });

  it('accepts human-edited selection only from selectable server tokens', () => {
    const run = detail();
    const parsed = parseFactoryTaskProposal(run.proposal?.normalizedJson);
    expect(parsed).toBeDefined();
    const draft = proposalDraft(parsed!);
    draft.selections[0]!.reason = '人工确认当前 ATP 和交期证据';
    const result = validateFactoryTaskProposalDraft({
      detail: run,
      draft,
  ...
  it('requires an audit reason only when the user changes the AI proposal', () => {
    ...
    const original = proposalDraft(
      parseFactoryTaskProposal(run.proposal?.normalizedJson)!,
    );
    const unchanged = validateFactoryTaskProposalDraft({
      detail: run,
      draft: original,
      sourcePlanId: '101',
      sourcePlanVersion: 4,
    ...
    const changedWithoutReason = validateFactoryTaskProposalDraft({
      detail: run,
      draft: original,
      sourcePlanId: '101',
      sourcePlanVersion: 4,
  ...
  it('fails closed for a non-selectable token, missing evidence, or stale source', () => {
    ...
    const draft = proposalDraft(
      parseFactoryTaskProposal(nonSelectable.proposal?.normalizedJson)!,
    );
  ...
  it('never posts authority quantities, MES ids, factory ids, or hashes as editable intent', () => {
    const run = detail();
    const draft = proposalDraft(
      parseFactoryTaskProposal(run.proposal?.normalizedJson)!,
    );
    ...
    expect(result.selections[0]).not.toHaveProperty('mesItemId');
