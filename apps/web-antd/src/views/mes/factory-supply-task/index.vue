...
import { getFactorySupplyTaskPage } from '#/api/mes/factory-supply-task';

import { factoryTaskStatusMeta } from './generation-policy';

defineOptions({ name: 'MesFactorySupplyTaskList' });
...
const list = ref<MesFactorySupplyTaskApi.BatchSummary[]>([]);
const total = ref(0);
const filters = reactive<MesFactorySupplyTaskApi.PageReq>({
  pageNo: 1,
  pageSize: 20,
});
...
async function load() {
  ...
  loading.value = true;
  try {
    const result = await getFactorySupplyTaskPage({
      ...filters,
...
function openDetail(id: string) {
  void router.push(`/mes/factory-supply-task/detail/${id}`);
}

function quantitySummary(record: {
  quantitySummary?: MesFactorySupplyTaskApi.BatchSummary['quantitySummary'];
}) {
...
<template>
  <Page
    ...
    description="由已确认履约需求生成的内部工厂供货任务草稿；正式生产工单仍由 MES 后续受控流程下达。"
    title="内部工厂供货任务"
  >
    <Alert
      v-if="!canQuery"
      description="缺少 mes:factory-supply-task:query 权限，页面不会尝试读取任何任务数据。"
    ...
    <template v-else>
      <Alert
        description="本页展示的是工厂供货任务草稿批次，不等同于 MES 正式生产工单。当前页面不提供直接下达、开工或完工操作。"
      ...
      <Card class="factory-task-list__filters" size="small">
        ...
        <Select
          ...
          :options="[
            ...
            { label: '已交接 MES', value: 'HANDED_OFF' },
        ...
        >
          <template #bodyCell="{ column, record }">
            ...
            <template v-else-if="column.key === 'status'">
              <Tag :color="factoryTaskStatusMeta(record.status).color">
                {{ factoryTaskStatusMeta(record.status).label }}
              </Tag>
            </template>
