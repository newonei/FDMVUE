...
export const FACTORY_TASK_AI_PERMISSION_CODES = [
  'mes:factory-supply-task:query',
  'mes:factory-supply-task:create',
  'mes:factory-supply-task:ai-generate',
  'mes:factory-supply-task:update',
  'fdmdocflow:generation:query',
  'fdmdocflow:generation:create',
  'fdmdocflow:generation:retry',
] as const;

const CONFIDENCES = new Set<MesFactorySupplyTaskApi.Confidence>([
  'HIGH',
  'LOW',
  'MEDIUM',
]);
...
export interface FactoryTaskSelectionDraft {
  confidence: MesFactorySupplyTaskApi.Confidence;
  factoryToken: string;
  lineToken: string;
  reason: string;
  riskCodes: string[];
...
export interface FactoryTaskProposalDraft {
  selections: FactoryTaskSelectionDraft[];
  summary: string;
}

export interface FactoryTaskProposalValidation {
  hasHumanOverrides: boolean;
  issues: string[];
  overrideReason?: string;
  selections: MesFactorySupplyTaskApi.MaterializeSelectionReq[];
  summary: string;
}
...
export function hasInternalFactoryDemand(
...
export function canGenerateFactorySupplyTask(
  detail: FdmWaimaoDemandPlanApi.Detail | undefined,
  hasPermission: (code: string) => boolean,
) {
  return Boolean(
    detail?.status === 'CONFIRMED' &&
    hasInternalFactoryDemand(detail) &&
    FACTORY_TASK_AI_PERMISSION_CODES.every((code) => hasPermission(code)),
  );
}
...
export function parseFactoryTaskProposal(
  normalizedJson?: null | string,
): MesFactorySupplyTaskApi.NormalizedProposal | undefined {
  ...
  try {
    ...
    const lineSelections: MesFactorySupplyTaskApi.ProposalSelection[] = [];
    for (const rawSelection of root.lineSelections) {
      ...
      const confidence = requiredString(selection.confidence) as
        | MesFactorySupplyTaskApi.Confidence
        | undefined;
      if (
        !lineToken ||
...
export function proposalDraft(
  proposal: MesFactorySupplyTaskApi.NormalizedProposal,
): FactoryTaskProposalDraft {
  return {
...
export function hasFactoryTaskProposalOverrides(
  proposal: MesFactorySupplyTaskApi.NormalizedProposal,
  draft: FactoryTaskProposalDraft,
) {
...
function candidateLines(detail: MesFactorySupplyTaskApi.GenerationDetail) {
  const lines = detail.proposal?.evidence?.candidateLines;
  return Array.isArray(lines) ? lines : [];
}

export function validateFactoryTaskProposalDraft(options: {
  detail: MesFactorySupplyTaskApi.GenerationDetail;
  draft: FactoryTaskProposalDraft;
  overrideReason?: string;
  sourcePlanId: string;
  sourcePlanVersion: number;
}): FactoryTaskProposalValidation {
  const { detail, draft, sourcePlanId, sourcePlanVersion } = options;
  const issues: string[] = [];
  const proposal = parseFactoryTaskProposal(detail.proposal?.normalizedJson);
  if (detail.status !== 'READY') issues.push('生成任务尚未处于 READY 状态');
  if (detail.generationType !== 'DEMAND_TO_FACTORY_TASK') {
  ...
  const hasHumanOverrides = proposal
    ? hasFactoryTaskProposalOverrides(proposal, draft)
    : false;
  ...
  const seen = new Set<string>();
  const selections: MesFactorySupplyTaskApi.MaterializeSelectionReq[] = [];
...
export function factoryTaskStatusMeta(status?: string) {
  const values: Record<string, { color: string; label: string }> = {
    ...
    HANDED_OFF: { color: 'green', label: '已交接 MES' },
  };
