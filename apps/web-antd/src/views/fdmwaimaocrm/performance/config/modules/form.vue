...
const formData = ref<FdmWaimaoCrmPerformanceConfigApi.PerformanceConfig>();
...
const [Modal, modalApi] = useVbenModal({
  ...
  async onOpenChange(isOpen: boolean) {
    ...
    const data =
      modalApi.getData<FdmWaimaoCrmPerformanceConfigApi.PerformanceConfig>();
    if (!data?.id) {
...
function buildSubmitData(values: Record<string, any>) {
  const data = {
    ...values,
  } as FdmWaimaoCrmPerformanceConfigApi.PerformanceConfig & {
    deptObjectId?: number;
    userObjectId?: number;
  };
  data.objectId =
