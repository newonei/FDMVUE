...
const formData = ref<FdmWaimaoCrmCustomerLimitConfigApi.CustomerLimitConfig>();
...
const [Modal, modalApi] = useVbenModal({
  async onConfirm() {
    ...
    // 提交表单
    const data =
      (await formApi.getValues()) as FdmWaimaoCrmCustomerLimitConfigApi.CustomerLimitConfig;
    try {
  ...
  async onOpenChange(isOpen: boolean) {
    ...
    // 加载数据
    let data =
      modalApi.getData<FdmWaimaoCrmCustomerLimitConfigApi.CustomerLimitConfig>();
    if (!data) {
      return;
    }
    if (data.type) {
