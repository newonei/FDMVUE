...
import { ProductEditTable } from '#/views/fdmwaimaocrm/product/components';

import { useFormSchema } from '../data';

const emit = defineEmits(['success']);
const formData = ref<FdmWaimaoCrmContractApi.Contract>();
...
function handleUpdateProducts(products: any) {
  formData.value = modalApi.getData<FdmWaimaoCrmContractApi.Contract>();
...
const [Modal, modalApi] = useVbenModal({
  async onConfirm() {
    ...
    // 提交表单
    const data =
      (await formApi.getValues()) as FdmWaimaoCrmContractApi.Contract;
  ...
  async onOpenChange(isOpen: boolean) {
    ...
    // 加载数据
    const data = modalApi.getData<FdmWaimaoCrmContractApi.Contract>();
    if (!data || !data.id) {
      return;
    }
    modalApi.lock();
