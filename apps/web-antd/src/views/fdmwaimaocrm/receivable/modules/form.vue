...
import { getContractSimpleList } from '#/api/fdmwaimaocrm/contract';
import {
  createReceivable,
  getReceivable,
  updateReceivable,
...
type ReceivablePrefillData = Partial<
  Pick<
    FdmWaimaoCrmReceivableApi.Receivable,
...
type ReceivableFormModalData = ReceivablePrefillData & {
  ...
  receivable?: Pick<FdmWaimaoCrmReceivableApi.Receivable, 'id'>;
};

const emit = defineEmits(['success']);
const formData = ref<Partial<FdmWaimaoCrmReceivableApi.Receivable>>();
...
async function buildCreateFormData(
  plan: ReceivablePrefillData,
): Promise<Partial<FdmWaimaoCrmReceivableApi.Receivable>> {
  const values: Partial<FdmWaimaoCrmReceivableApi.Receivable> = {
...
const [Modal, modalApi] = useVbenModal({
  async onConfirm() {
    ...
    // 提交表单
    const data =
      (await formApi.getValues()) as FdmWaimaoCrmReceivableApi.Receivable;
    try {
