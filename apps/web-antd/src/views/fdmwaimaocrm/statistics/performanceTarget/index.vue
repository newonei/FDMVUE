...
import type { FdmWaimaoCrmStatisticsPerformanceTargetApi } from '#/api/fdmwaimaocrm/statistics/performanceTarget';
...
import { getPerformanceTargetSummary } from '#/api/fdmwaimaocrm/statistics/performanceTarget';
...
const [Grid, gridApi] = useVbenVxeGrid({
  ...
  } as VxeTableGridOptions<FdmWaimaoCrmStatisticsPerformanceTargetApi.PerformanceTargetRespVO>,
});

/** 获取接口参数 */
async function getApiParams() {
  const values =
    await formApi.getValues<FdmWaimaoCrmStatisticsPerformanceTargetApi.PerformanceTargetReqVO>();
  return {
    ...values,
...
function getChartOptions(
  year: number,
  data: FdmWaimaoCrmStatisticsPerformanceTargetApi.PerformanceTargetRespVO[],
): EChartsOption {
  return {
    grid: {
      left: 20,
