...
import type { FdmWaimaoCrmStatisticsProductApi } from '#/api/fdmwaimaocrm/statistics/product';
...
import { getProductSimpleList } from '#/api/fdmwaimaocrm/product';
import { getProductCategoryList } from '#/api/fdmwaimaocrm/product/category';
import {
  getProductCategorySummary,
  getProductSalesList,
} from '#/api/fdmwaimaocrm/statistics/product';
...
type ProductSalesRow =
  Partial<FdmWaimaoCrmStatisticsProductApi.ProductSalesRespVO> & {
...
const [ProductCategoryGrid, productCategoryGridApi] = useVbenVxeGrid({
  ...
  } as VxeTableGridOptions<FdmWaimaoCrmStatisticsProductApi.ProductCategoryRespVO>,
});
...
function getCategoryKey(
  item: FdmWaimaoCrmStatisticsProductApi.ProductSalesRespVO,
) {
...
function buildProductSummaryRow(
  rows: FdmWaimaoCrmStatisticsProductApi.ProductSalesRespVO[],
): ProductSalesRow {
  return {
...
function buildProductGroups(
  ...
  const result: FdmWaimaoCrmStatisticsProductApi.ProductSalesRespVO[][] = [];
  let index = 0;
  while (index < rows.length) {
    const productKey = getProductKey(rows[index]!);
    const productRows: FdmWaimaoCrmStatisticsProductApi.ProductSalesRespVO[] =
      [];
...
/** 构建列表展示数据 */
function buildList(
  data: FdmWaimaoCrmStatisticsProductApi.ProductSalesRespVO[],
): ProductSalesRow[] {
  ...
  while (index < data.length) {
    ...
    const categoryRows: FdmWaimaoCrmStatisticsProductApi.ProductSalesRespVO[] =
      [];
...
function getProductCategoryChartOptions(
  data: FdmWaimaoCrmStatisticsProductApi.ProductCategoryRespVO[],
): EChartsOption {
  return {
    title: {
      text: '产品分类销量占比',
...
function openContract(id?: number) {
  ...
  push({ name: 'FdmWaimaoCrmContractDetail', params: { id } });
}
...
function openCustomer(id?: number) {
  ...
  push({ name: 'FdmWaimaoCrmCustomerDetail', params: { id } });
}
...
function openProduct(id?: number) {
  ...
  push({ name: 'FdmWaimaoCrmProductDetail', params: { id } });
}
