...
const formData = ref<FdmWaimaoCrmBusinessStatusApi.BusinessStatus>();
...
const [Modal, modalApi] = useVbenModal({
  async onConfirm() {
    ...
    // 提交表单
    const data =
      (await formApi.getValues()) as FdmWaimaoCrmBusinessStatusApi.BusinessStatus;
    try {
  ...
  async onOpenChange(isOpen: boolean) {
    ...
    // 加载数据
    const data =
      modalApi.getData<FdmWaimaoCrmBusinessStatusApi.BusinessStatus>();
    modalApi.lock();
    try {
      if (!data || !data.id) {
        formData.value = {
