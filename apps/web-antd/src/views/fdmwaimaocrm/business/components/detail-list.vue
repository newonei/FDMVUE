...
import type { FdmWaimaoCrmContactApi } from '#/api/fdmwaimaocrm/contact';
...
} from '#/api/fdmwaimaocrm/business';
import {
  createContactBusinessList,
  deleteContactBusinessList,
} from '#/api/fdmwaimaocrm/contact';
...
const checkedRows = ref<FdmWaimaoCrmBusinessApi.Business[]>([]);
function setCheckedRows({
  records,
}: {
  records: FdmWaimaoCrmBusinessApi.Business[];
}) {
...
/** 查看商机详情 */
function handleDetail(row: FdmWaimaoCrmBusinessApi.Business) {
  push({ name: 'FdmWaimaoCrmBusinessDetail', params: { id: row.id } });
}

/** 查看客户详情 */
function handleCustomerDetail(row: FdmWaimaoCrmBusinessApi.Business) {
...
async function handleCreateContactBusinessList(businessIds: number[]) {
  const data = {
    contactId: props.bizId,
    businessIds,
  } as FdmWaimaoCrmContactApi.ContactBusinessReqVO;
...
const [Grid, gridApi] = useVbenVxeGrid({
  ...
  } as VxeTableGridOptions<FdmWaimaoCrmBusinessApi.Business>,
  gridEvents: {
          ...
          :actions="[
            {
              ...
              auth: ['fdmwaimaocrm:business:create'],
            ...
            },
            {
              ...
              icon: ACTION_ICON.ADD,
              type: 'default',
              auth: ['fdmwaimaocrm:contact:create-business'],
              ifShow: () => !!contactId,
