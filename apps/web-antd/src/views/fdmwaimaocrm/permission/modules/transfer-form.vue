...
import { transferBusiness } from '#/api/fdmwaimaocrm/business';
import { transferClue } from '#/api/fdmwaimaocrm/clue';
import { transferContact } from '#/api/fdmwaimaocrm/contact';
import { transferContract } from '#/api/fdmwaimaocrm/contract';
import { transferCustomer } from '#/api/fdmwaimaocrm/customer';
...
const [Modal, modalApi] = useVbenModal({
  async onConfirm() {
    ...
    // 提交表单
    const data =
      (await formApi.getValues()) as FdmWaimaoCrmPermissionApi.BusinessTransferReqVO;
    try {
      switch (bizType.value) {
        case BizTypeEnum.CRM_BUSINESS: {
