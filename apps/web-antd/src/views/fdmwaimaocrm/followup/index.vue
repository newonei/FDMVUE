...
import type { FdmWaimaoCrmFollowUpApi } from '#/api/fdmwaimaocrm/followup';
...
} from '#/api/fdmwaimaocrm/followup';
...
/** 删除跟进记录 */
async function handleDelete(row: FdmWaimaoCrmFollowUpApi.FollowUpRecord) {
...
function openContactDetail(id: number) {
  push({ name: 'FdmWaimaoCrmContactDetail', params: { id } });
}

/** 打开商机详情 */
function openBusinessDetail(id: number) {
  push({ name: 'FdmWaimaoCrmBusinessDetail', params: { id } });
}
...
const [Grid, gridApi] = useVbenVxeGrid({
  ...
  } as VxeTableGridOptions<FdmWaimaoCrmFollowUpApi.FollowUpRecord>,
});
