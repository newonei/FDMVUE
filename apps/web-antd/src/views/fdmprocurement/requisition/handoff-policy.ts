...
export interface ErpLifecycleNotice {
  description: string;
  message: string;
  type: 'error' | 'info' | 'success' | 'warning';
}
...
export function erpLifecycleActionMeta(
  action: FdmProcurementPurchaseOrderHandoffApi.LifecycleAction,
) {
  const values = {
...
export function erpLifecycleEventResultMeta(
  result: FdmProcurementPurchaseOrderHandoffApi.LifecycleProcessingResult,
) {
  const values = {
...
export function handoffStatusMeta(
  status: FdmProcurementPurchaseOrderHandoffApi.HandoffStatus,
) {
  const values = {
    MANUAL_REVIEW: { color: 'red', label: '创建交付需人工处理' },
    PROCESSING: { color: 'processing', label: '正在创建 ERP 草稿' },
    READY: { color: 'blue', label: '等待创建 ERP 草稿' },
    RETRY_WAIT: { color: 'orange', label: '创建交付等待自动重试' },
    SUCCESS: { color: 'green', label: '创建交付成功' },
  } as const;
  return values[status] ?? { color: 'default', label: status || '未知' };
}

export function erpLifecycleStatusMeta(status?: null | string) {
  const normalized = String(status || '')
    .trim()
    .toUpperCase();
  const values = {
    APPROVED: { color: 'green', label: 'ERP 已确认' },
    CANCELLED: { color: 'red', label: 'ERP 已取消' },
    CONFIRMED: { color: 'green', label: 'ERP 已确认' },
    DRAFT: { color: 'blue', label: 'ERP 草稿已创建' },
  } as const;
  if (!normalized) {
    return { color: 'default', label: '尚未取得 ERP 生命周期' };
  }
  return (
    values[normalized as keyof typeof values] ?? {
      color: 'orange',
      label: `ERP 状态待识别（${normalized}）`,
    }
  );
}

function displayErpLifecycleDateTime(
...
export function erpLifecycleNotice(
  handoff: FdmProcurementPurchaseOrderHandoffApi.Handoff,
): ErpLifecycleNotice | undefined {
  if (
    String(handoff.erpPurchaseOrderStatus || '').toUpperCase() !== 'CANCELLED'
  ) {
    return undefined;
  }
  const details = [
    `取消时间 ${displayErpLifecycleDateTime(handoff.erpStatusUpdatedAt)}`,
    `取消原因 ${String(handoff.erpCancelReason || '').trim() || 'ERP 未提供'}`,
  ];
  if (handoff.erpLastActorUserId) {
    details.push(`操作人 ID ${handoff.erpLastActorUserId}`);
  }
  return {
...
export function canRetryHandoff(
  ...
  return (
    handoff.status === 'MANUAL_REVIEW' &&
    hasPermission('fdmprocurement:purchase-order-handoff:retry')
  );
}

export function canRetryProjection(
  state: FdmProcurementPurchaseOrderHandoffApi.ProjectionState,
  hasPermission: (code: string) => boolean,
) {
  return (
...
export function canLoadLifecycleHistory(
  handoffId: string,
  handoffs: readonly FdmProcurementPurchaseOrderHandoffApi.Handoff[],
  hasQueryPermission: boolean,
) {
  return (
    hasQueryPermission &&
    handoffId.length > 0 &&
    handoffs.some((handoff) => handoff.id === handoffId)
  );
}
...
export function executionDocumentTypeMeta(type?: null | string) {
  const normalized = String(type || '')
    ...
    .toUpperCase();
  const values = {
    PURCHASE_IN: { color: 'blue', label: '采购入库' },
...
export function executionActionMeta(
  action: FdmProcurementPurchaseOrderHandoffApi.ExecutionAction,
) {
  const values = {
...
export function executionEventResultMeta(
  result: FdmProcurementPurchaseOrderHandoffApi.ExecutionProcessingResult,
) {
  const values = {
    APPLIED: { color: 'green', label: '已生效' },
    IGNORED_STALE: { color: 'default', label: '过期事件已忽略' },
...
export function projectionStatusMeta(
  status: FdmProcurementPurchaseOrderHandoffApi.ProjectionStatus,
) {
  const values = {
...
export function projectionStateNotice(
  ...
  if (state.status === 'PUBLISHED') {
    const publishedWithoutHandoff =
      state.handoffCount === 0 ||
      (reconciliation.handoffListLoaded &&
        reconciliation.visibleHandoffCount === 0);
    if (publishedWithoutHandoff) {
      return {
        description:
          '审批事件已经标记为已发布，但当前没有任何采购单交接台账。这是后端投递与投影的一致性异常，页面不会伪造台账或发起补投，请由运营排查。',
        message: '投递已发布但未生成 ERP 交接台账',
        type: 'error',
      };
    }
    return {
      description: `审批事件已发布，并生成 ${state.handoffCount} 个 ERP 采购单交接台账。`,
      message: '审批事件投递完成',
      type: 'success',
    };
  }
  if (state.status === 'DEAD_LETTER') {
    return {
      description: `死信时间 ${displayProjectionDateTime(state.deadLetterAt)}；${displayProjectionError(state)}。具备权限的运营人员可填写可审计原因，将该事件受控恢复到等待投递；页面不会直接执行投影或伪造交接台账。`,
      message: '审批事件投递进入死信',
      type: 'error',
    };
  }
  ...
  if (state.status === 'PROCESSING') {
    return {
      description:
        '后端 Worker 正在把审批冻结快照投影为 ERP 草稿采购单交接台账，请等待处理完成。',
      message: '审批事件正在投影',
      type: 'info',
    };
  }
