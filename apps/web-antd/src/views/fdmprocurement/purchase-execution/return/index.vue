...
import type { FdmProcurementPurchaseExecutionReturnApi } from '#/api/fdmprocurement/purchase-execution/return';
...
} from '#/api/fdmprocurement/purchase-execution/return';

import { validatePurchaseReturnReverseReason } from './status-policy';

defineOptions({ name: 'FdmProcurementPurchaseExecutionReturn' });

type PurchaseReturn = FdmProcurementPurchaseExecutionReturnApi.PurchaseReturn;
...
const companyId = ref('');
const status = ref<FdmProcurementPurchaseExecutionReturnApi.PostingStatus>();
const editorOpen = ref(false);
const reverseOpen = ref(false);
const editingId = ref<string>();
const editingVersion = ref<number>();
const reversing = ref<PurchaseReturn>();
...
async function load() {
  ...
  loading.value = true;
  try {
    const result = await getPurchaseReturnPage({
...
async function openEdit(row: PurchaseReturn | Record<string, any>) {
  row = row as PurchaseReturn;
  if (!row.id) return;
...
function parseLines() {
  ...
  return value as FdmProcurementPurchaseExecutionReturnApi.LineSaveReq[];
}

async function save() {
  if (!form.purchaseOrderId.trim() || !form.documentTime) {
    message.warning('请输入采购单 ID 和退货时间');
    return;
  }
  let lines: FdmProcurementPurchaseExecutionReturnApi.LineSaveReq[];
  try {
    lines = parseLines();
    if (lines.length === 0) throw new TypeError('至少需要一条退货明细');
  } catch (error) {
    message.error(error instanceof Error ? error.message : '明细 JSON 无效');
    return;
  }
  const payload: FdmProcurementPurchaseExecutionReturnApi.SaveReq = {
  ...
  try {
    ...
    message.success(editingId.value ? '采购退货已更新' : '采购退货已创建');
...
async function post(row: PurchaseReturn | Record<string, any>) {
  row = row as PurchaseReturn;
  ...
  message.success('采购退货已过账');
  await load();
}

function openReverse(row: PurchaseReturn | Record<string, any>) {
  reversing.value = row as PurchaseReturn;
  reverseReason.value = '';
...
async function reverse() {
  ...
  const result = validatePurchaseReturnReverseReason(reverseReason.value);
  if (!result.valid) {
...
async function remove(row: PurchaseReturn | Record<string, any>) {
  row = row as PurchaseReturn;
  await deletePurchaseReturn(row.id);
  message.success('采购退货已删除');
  await load();
}

async function download() {
  const data = await exportPurchaseReturn({
    companyId: companyId.value.trim() || undefined,
    pageNo: 1,
    pageSize: pageSize.value,
    status: status.value,
  });
  downloadFileFromBlobPart({ fileName: 'FDM采购退货.xls', source: data });
}
  ...
  <Page auto-content-height>
    ...
    >
      <div class="editor">
        ...
        <label class="wide">
          退货明细（purchaseOrderLineId、warehouseId、quantity）
