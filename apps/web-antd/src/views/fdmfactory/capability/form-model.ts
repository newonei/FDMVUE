...
export interface CapabilityFormDraft {
  ...
  evidenceMode: FdmFactoryCapabilityApi.EvidenceMode;
  evidenceNote: string;
  ...
  status: FdmFactoryCapabilityApi.CapabilityStatus;
...
export function buildCapabilityPayload(
  ...
  if (!POSITIVE_ID.test(productId)) errors.push('产品 ID 必须是正整数');
  if (!POSITIVE_ID.test(skuId)) errors.push('产品 SKU ID 必须是正整数');
