...
import { getSupplyTask } from '#/api/fdmfactory/supply-task';
import {
  atpStatusMeta,
  supplyTaskStatusMeta,
} from '#/views/fdmfactory/supply-task/generation-policy';

defineOptions({ name: 'FdmFactorySupplyTaskDetail' });

const route = useRoute();
const router = useRouter();
const { hasAccessByCodes } = useAccess();
const detail = ref<FdmFactorySupplyTaskApi.BatchDetail>();
...
async function load() {
  ...
  loading.value = true;
  try {
    const result = await getSupplyTask(id.value);
...
function back() {
  void router.push('/fdmfactory/supply-task');
}
...
<template>
  ...
  >
    <template #extra>
      ...
      <Tag v-if="detail" :color="supplyTaskStatusMeta(detail.status).color">
        {{ supplyTaskStatusMeta(detail.status).label }}
      </Tag>
    </template>

    <Alert
      v-if="!canQuery"
      description="缺少 fdmfactory:supply-task:query 权限，页面不会请求任务详情。"
      message="无权查看该工厂供货任务"
      show-icon
      type="error"
    />
    ...
    <div v-else class="factory-task-detail">
      <Alert
        description="这里保存的是经人工确认的 FDM 工厂供货任务草稿，不是正式生产工单。本页面只展示冻结事实和审计证据，不提供直接下达、开工或完工操作。"
      ...
      >
        <template #title>
          <div class="factory-task-detail__task-title">
            <div>
              <Tag :color="supplyTaskStatusMeta(task.status).color">
                {{ supplyTaskStatusMeta(task.status).label }}
              </Tag>
