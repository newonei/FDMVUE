...
import type { SupplyTaskProposalDraft } from '../generation-policy';
...
import {
  getSupplyTaskGeneration,
  materializeSupplyTaskGeneration,
  regenerateSupplyTaskGeneration,
  retrySupplyTaskGeneration,
  searchSupplyTaskAiModels,
  startSupplyTaskGeneration,
} from '#/api/fdmfactory/supply-task';
import {
...
defineOptions({ name: 'FdmFactorySupplyTaskGenerationDrawer' });
...
const run = ref<FdmFactorySupplyTaskApi.GenerationDetail>();
const draft = ref<SupplyTaskProposalDraft>();
...
const canUse = computed(() =>
  canGenerateSupplyTask(props.plan, (code) => hasAccessByCodes([code])),
);
const modelCatalog = useAiModelCatalog({
  actionCode: 'fdmfactory.demand-to-supply-task',
  load: searchSupplyTaskAiModels,
...
const stage = computed(() => {
  const values: Partial<
    Record<FdmFactorySupplyTaskApi.GenerationStatus, AiGenerationJob['stage']>
  > = {
...
const draftValidation = computed(() => {
  ...
  return validateSupplyTaskProposalDraft({
    detail: run.value,
    draft: draft.value,
...
function terminal(status: FdmFactorySupplyTaskApi.GenerationStatus) {
  return [
    'CANCELLED',
    'EXPIRED',
    'FAILED',
...
function initializeDraft(value: FdmFactorySupplyTaskApi.GenerationDetail) {
  if (value.status !== 'READY') return;
  overrideReason.value = '';
  const parsed = parseSupplyTaskProposal(value.proposal?.normalizedJson);
  if (!parsed) {
...
async function loadRun(runId: string, version = requestVersion) {
  try {
    const result = await getSupplyTaskGeneration(runId);
...
async function start() {
  ...
  draft.value = undefined;
  try {
    const ticket = await startSupplyTaskGeneration({
...
async function retry() {
  ...
  loadingRun.value = true;
  try {
    const ticket = await retrySupplyTaskGeneration(
      run.value.runId,
      run.value.version,
    );
    await loadRun(ticket.runId, version);
...
async function regenerate() {
  ...
  draft.value = undefined;
  try {
    const ticket = await regenerateSupplyTaskGeneration(run.value.runId, {
...
function updateSelection(
  index: number,
  patch: Partial<SupplyTaskProposalDraft['selections'][number]>,
) {
...
function capabilityAuthority(
  candidate: FdmFactorySupplyTaskApi.FactoryCandidateEvidence,
) {
  const value = candidate.factoryCapabilityEvidence;
  if (!value || !('capabilityId' in value) || !value.capabilityId) {
    return undefined;
  }
  return value as FdmFactorySupplyTaskApi.FactoryCapabilityAuthoritySnapshot;
}
...
function capabilityRequirementGroups(
  authority?: FdmFactorySupplyTaskApi.FactoryCapabilityAuthoritySnapshot,
) {
  if (!authority) return [];
  return [
...
async function materialize() {
  ...
  try {
    const result = await materializeSupplyTaskGeneration({
    ...
    await router.push(`/fdmfactory/supply-task/detail/${result.batch.id}`);
  } finally {
...
<template>
  ...
  >
    ...
    <div v-else class="factory-task-generation">
      <Alert
        description="产品、SKU、数量、单位、交期、FDM 工厂物料映射和 ATP 上限均由服务端冻结。AI 与人工只能在服务端签发的可用厂区 Token 中选择并补充理由、风险和信心；确认后不会直接下达正式生产工单。"
      ...
      <AiGenerationStartPanel
        ...
        source-description="重新读取当前已确认履约计划、FDM 产品映射、厂区准入和 FDM 仓储 ATP，再由所选模型提出厂区选择建议。"
      ...
      <template v-if="ready && run && draft">
        ...
        >
          ...
          <Descriptions :column="4" size="small">
            ...
            <Descriptions.Item label="FDM 产品身份">
              产品
              {{
                candidateLines.find(
                  (line) => line.lineToken === selection.lineToken,
              ...
              <Select
                ...
                @update:value="
                  updateSelection(index, {
                    confidence: $event as FdmFactorySupplyTaskApi.Confidence,
                  })
                "
              />
            </label>
                ...
                >
                  ...
                  <Descriptions :column="2" size="small">
                    <Descriptions.Item label="能力 / SKU ID">
                      ...
                      {{ capabilityAuthority(candidate)?.skuId }}
                    </Descriptions.Item>
