...
async function loadAccessories() {
  ...
  const rows = res.list ?? [];
  accessoryOptions.value = rows.map(
    (r: FdmdataDataJustAccessoryApi.Accessory) => ({
      value: r.id!,
...
async function openEdit(row: Partial<StandardComboSpecTemplateApi.Row>) {
...
async function handleDelete(row: Partial<StandardComboSpecTemplateApi.Row>) {
  Modal.confirm({
    title: '确认删除该对照行？',
    onOk: async () => {
...
const columns: ColumnsType = [
  {
    title: '配件编码',
    dataIndex: 'accessoryItemCode',
    width: 140,
    ellipsis: true,
  },
  {
    title: '配件名称',
    dataIndex: 'accessoryProductName',
    width: 200,
    ellipsis: true,
  },
  {
...
<template>
  <Page
    auto-content-height
    description="用于标准组合生成：配件名称里含误导性「长*宽」时，在此维护该配件实际可配的瑜伽垫等成品规格键（与成品列表解析规则一致：优先长*宽*厚，可退回长*宽）。"
  >
    ...
    >
      <Form layout="vertical" class="mt-2">
        ...
        <FormItem
          label="成品规格键：长*宽"
          extra="如 183*61，与成品列表「匹配键(长*宽)」一致"
        >
          <Input
            v-model:value="editForm.specLwKey"
            placeholder="183*61"
            allow-clear
          />
        </FormItem>
        <FormItem
          label="成品规格键：长*宽*厚（可选）"
          extra="若只按长宽匹配可留空"
        >
          <Input
            v-model:value="editForm.specFullKey"
            placeholder="183*61*0.6"
            allow-clear
          />
        </FormItem>
        <FormItem label="备注">
