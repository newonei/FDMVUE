...
const CALC_HELP_BY_LABEL: Record<string, string> = {
  ...
  实际销售额:
    '实际销售额 = 金额计算基数 - 退款金额 - 刷单本金；淘宝/抖音金额计算基数为成交额(GMV)，其他平台为已支付金额。',
  真实净销售额:
    '真实净销售额 = 净销售额 = 金额计算基数 - 退款金额 - 刷单本金；淘宝/抖音金额计算基数为成交额(GMV)，其他平台为已支付金额。',
  真实支付订单数:
    '真实支付订单数 = 已支付订单笔数 - 刷单单量，结果小于 0 时按 0 处理。',
  真实支付金额:
    '真实支付金额 = 金额计算基数 - 退款金额 - 刷单本金；淘宝/抖音金额计算基数为成交额(GMV)，其他平台为已支付金额。',
  真实订单数:
  ...
  退款占比: '退款占比：淘宝/抖音 = 退款金额 / 成交额(GMV) × 100%；其他平台 = 退款金额 / 支付金额 × 100%。',
  退款率: '退款率：淘宝/抖音 = 退款金额 / 成交额(GMV) × 100%；其他平台 = 退款金额 / 支付金额 × 100%。',
  '退款率(支付时间)':
  ...
  净销售额: '净销售额：淘宝/抖音 = 成交额(GMV) - 退款金额 - 刷单本金；其他平台 = 已支付金额 - 退款金额 - 刷单本金。',
  '刷单总成本（加平台扣点）':
