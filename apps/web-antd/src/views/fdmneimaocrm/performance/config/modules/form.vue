...
const formData = ref<FdmNeimaoCrmPerformanceConfigApi.PerformanceConfig>();
...
const [Modal, modalApi] = useVbenModal({
  ...
  async onOpenChange(isOpen: boolean) {
    ...
    const data =
      modalApi.getData<FdmNeimaoCrmPerformanceConfigApi.PerformanceConfig>();
    if (!data?.id) {
...
function buildSubmitData(values: Record<string, any>) {
  const data = {
    ...values,
  } as FdmNeimaoCrmPerformanceConfigApi.PerformanceConfig & {
    deptObjectId?: number;
    userObjectId?: number;
  };
  data.objectId =
