...
import type { FdmNeimaoCrmFollowUpApi } from '#/api/fdmneimaocrm/followup';
...
} from '#/api/fdmneimaocrm/followup';
...
/** 删除跟进记录 */
async function handleDelete(row: FdmNeimaoCrmFollowUpApi.FollowUpRecord) {
...
function openContactDetail(id: number) {
  push({ name: 'FdmNeimaoCrmContactDetail', params: { id } });
}

/** 打开商机详情 */
function openBusinessDetail(id: number) {
  push({ name: 'FdmNeimaoCrmBusinessDetail', params: { id } });
}
...
const [Grid, gridApi] = useVbenVxeGrid({
  ...
  } as VxeTableGridOptions<FdmNeimaoCrmFollowUpApi.FollowUpRecord>,
});
