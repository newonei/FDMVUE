...
import type { FdmNeimaoCrmReceivablePlanApi } from '#/api/fdmneimaocrm/receivable/plan';
...
} from '#/api/fdmneimaocrm/receivable/plan';
...
/** 编辑回款计划 */
function handleEdit(row: FdmNeimaoCrmReceivablePlanApi.Plan) {
  formModalApi.setData(row).open();
}

/** 删除回款计划 */
async function handleDelete(row: FdmNeimaoCrmReceivablePlanApi.Plan) {
...
/** 创建回款计划 */
function handleCreateReceivable(row: FdmNeimaoCrmReceivablePlanApi.Plan) {
  receivableFormModalApi.setData({ plan: row }).open();
}

/** 查看回款计划详情 */
function handleDetail(row: FdmNeimaoCrmReceivablePlanApi.Plan) {
  push({ name: 'FdmNeimaoCrmReceivablePlanDetail', params: { id: row.id } });
}

/** 查看客户详情 */
function handleCustomerDetail(row: FdmNeimaoCrmReceivablePlanApi.Plan) {
...
const [Grid, gridApi] = useVbenVxeGrid({
  ...
  } as VxeTableGridOptions<FdmNeimaoCrmReceivablePlanApi.Plan>,
});
</script>
          ...
          :actions="[
            {
              ...
              auth: ['fdmneimaocrm:receivable-plan:create'],
            ...
            },
            {
              ...
              auth: ['fdmneimaocrm:receivable-plan:export'],
          ...
          :actions="[
            {
              ...
              type: 'link',
              icon: ACTION_ICON.ADD,
              auth: ['fdmneimaocrm:receivable:create'],
            ...
            },
            {
              ...
              auth: ['fdmneimaocrm:receivable-plan:update'],
            ...
            },
            {
              ...
              auth: ['fdmneimaocrm:receivable-plan:delete'],
              popConfirm: {
