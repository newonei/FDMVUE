...
import { getContractSimpleList } from '#/api/fdmneimaocrm/contract';
import {
  createReceivable,
  getReceivable,
  updateReceivable,
} from '#/api/fdmneimaocrm/receivable';
...
type ReceivablePrefillData = Partial<
  Pick<
    FdmNeimaoCrmReceivableApi.Receivable,
...
type ReceivableFormModalData = ReceivablePrefillData & {
  ...
  receivable?: Pick<FdmNeimaoCrmReceivableApi.Receivable, 'id'>;
};

const emit = defineEmits(['success']);
const formData = ref<Partial<FdmNeimaoCrmReceivableApi.Receivable>>();
...
async function buildCreateFormData(
  plan: ReceivablePrefillData,
): Promise<Partial<FdmNeimaoCrmReceivableApi.Receivable>> {
  const values: Partial<FdmNeimaoCrmReceivableApi.Receivable> = {
...
const [Modal, modalApi] = useVbenModal({
  async onConfirm() {
    ...
    // 提交表单
    const data =
      (await formApi.getValues()) as FdmNeimaoCrmReceivableApi.Receivable;
    try {
