...
const formData = ref<FdmNeimaoCrmCustomerLimitConfigApi.CustomerLimitConfig>();
...
const [Modal, modalApi] = useVbenModal({
  async onConfirm() {
    ...
    // 提交表单
    const data =
      (await formApi.getValues()) as FdmNeimaoCrmCustomerLimitConfigApi.CustomerLimitConfig;
    try {
  ...
  async onOpenChange(isOpen: boolean) {
    ...
    // 加载数据
    let data =
      modalApi.getData<FdmNeimaoCrmCustomerLimitConfigApi.CustomerLimitConfig>();
    if (!data) {
      return;
    }
    if (data.type) {
