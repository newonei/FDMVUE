...
const formData = ref<FdmNeimaoCrmContractApi.Contract>();
...
function handleUpdateProducts(products: any) {
  formData.value = modalApi.getData<FdmNeimaoCrmContractApi.Contract>();
...
const [Modal, modalApi] = useVbenModal({
  async onConfirm() {
    ...
    // 提交表单
    const data =
      (await formApi.getValues()) as FdmNeimaoCrmContractApi.Contract;
  ...
  async onOpenChange(isOpen: boolean) {
    ...
    // 加载数据
    const data = modalApi.getData<FdmNeimaoCrmContractApi.Contract>();
    if (!data || !data.id) {
      return;
    }
    modalApi.lock();
