...
import type { FdmNeimaoCrmStatisticsPerformanceTargetApi } from '#/api/fdmneimaocrm/statistics/performanceTarget';
...
import { getPerformanceTargetSummary } from '#/api/fdmneimaocrm/statistics/performanceTarget';
...
const [Grid, gridApi] = useVbenVxeGrid({
  ...
  } as VxeTableGridOptions<FdmNeimaoCrmStatisticsPerformanceTargetApi.PerformanceTargetRespVO>,
});

/** 获取接口参数 */
async function getApiParams() {
  const values =
    await formApi.getValues<FdmNeimaoCrmStatisticsPerformanceTargetApi.PerformanceTargetReqVO>();
  return {
    ...values,
...
function getChartOptions(
  year: number,
  data: FdmNeimaoCrmStatisticsPerformanceTargetApi.PerformanceTargetRespVO[],
): EChartsOption {
  return {
    grid: {
      left: 20,
