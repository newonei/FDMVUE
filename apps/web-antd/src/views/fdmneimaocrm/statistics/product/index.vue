...
import type { FdmNeimaoCrmStatisticsProductApi } from '#/api/fdmneimaocrm/statistics/product';
...
import { getProductSimpleList } from '#/api/fdmneimaocrm/product';
import { getProductCategoryList } from '#/api/fdmneimaocrm/product/category';
import {
  getProductCategorySummary,
  getProductSalesList,
} from '#/api/fdmneimaocrm/statistics/product';
...
type ProductSalesRow =
  Partial<FdmNeimaoCrmStatisticsProductApi.ProductSalesRespVO> & {
...
const [ProductCategoryGrid, productCategoryGridApi] = useVbenVxeGrid({
  ...
  } as VxeTableGridOptions<FdmNeimaoCrmStatisticsProductApi.ProductCategoryRespVO>,
});
...
function getCategoryKey(
  item: FdmNeimaoCrmStatisticsProductApi.ProductSalesRespVO,
) {
...
function buildProductSummaryRow(
  rows: FdmNeimaoCrmStatisticsProductApi.ProductSalesRespVO[],
): ProductSalesRow {
  return {
...
function buildProductGroups(
  ...
  const result: FdmNeimaoCrmStatisticsProductApi.ProductSalesRespVO[][] = [];
  let index = 0;
  while (index < rows.length) {
    const productKey = getProductKey(rows[index]!);
    const productRows: FdmNeimaoCrmStatisticsProductApi.ProductSalesRespVO[] =
      [];
...
/** 构建列表展示数据 */
function buildList(
  data: FdmNeimaoCrmStatisticsProductApi.ProductSalesRespVO[],
): ProductSalesRow[] {
  ...
  while (index < data.length) {
    ...
    const categoryRows: FdmNeimaoCrmStatisticsProductApi.ProductSalesRespVO[] =
      [];
...
function getProductCategoryChartOptions(
  data: FdmNeimaoCrmStatisticsProductApi.ProductCategoryRespVO[],
): EChartsOption {
  return {
    title: {
      text: '产品分类销量占比',
...
function openContract(id?: number) {
  ...
  push({ name: 'FdmNeimaoCrmContractDetail', params: { id } });
}
...
function openCustomer(id?: number) {
  ...
  push({ name: 'FdmNeimaoCrmCustomerDetail', params: { id } });
}
...
function openProduct(id?: number) {
  ...
  push({ name: 'FdmNeimaoCrmProductDetail', params: { id } });
}
