...
import { transferBusiness } from '#/api/fdmneimaocrm/business';
import { transferClue } from '#/api/fdmneimaocrm/clue';
import { transferContact } from '#/api/fdmneimaocrm/contact';
import { transferContract } from '#/api/fdmneimaocrm/contract';
import { transferCustomer } from '#/api/fdmneimaocrm/customer';
...
const [Modal, modalApi] = useVbenModal({
  async onConfirm() {
    ...
    // 提交表单
    const data =
      (await formApi.getValues()) as FdmNeimaoCrmPermissionApi.BusinessTransferReqVO;
    try {
      switch (bizType.value) {
        case BizTypeEnum.CRM_BUSINESS: {
