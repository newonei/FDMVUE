...
import type { FdmNeimaoCrmContactApi } from '#/api/fdmneimaocrm/contact';
...
} from '#/api/fdmneimaocrm/business';
import {
  createContactBusinessList,
  deleteContactBusinessList,
} from '#/api/fdmneimaocrm/contact';
...
const checkedRows = ref<FdmNeimaoCrmBusinessApi.Business[]>([]);
function setCheckedRows({
  records,
}: {
  records: FdmNeimaoCrmBusinessApi.Business[];
}) {
...
/** 查看商机详情 */
function handleDetail(row: FdmNeimaoCrmBusinessApi.Business) {
  push({ name: 'FdmNeimaoCrmBusinessDetail', params: { id: row.id } });
}

/** 查看客户详情 */
function handleCustomerDetail(row: FdmNeimaoCrmBusinessApi.Business) {
...
async function handleCreateContactBusinessList(businessIds: number[]) {
  const data = {
    contactId: props.bizId,
    businessIds,
  } as FdmNeimaoCrmContactApi.ContactBusinessReqVO;
...
const [Grid, gridApi] = useVbenVxeGrid({
  ...
  } as VxeTableGridOptions<FdmNeimaoCrmBusinessApi.Business>,
  gridEvents: {
          ...
          :actions="[
            {
              ...
              auth: ['fdmneimaocrm:business:create'],
            ...
            },
            {
              ...
              icon: ACTION_ICON.ADD,
              type: 'default',
              auth: ['fdmneimaocrm:contact:create-business'],
              ifShow: () => !!contactId,
