...
import { ProductEditTable } from '#/views/fdmneimaocrm/product/components';

import { useFormSchema } from '../data';

const emit = defineEmits(['success']);
const formData = ref<FdmNeimaoCrmBusinessApi.Business>();
...
function handleUpdateProducts(products: any) {
  formData.value = modalApi.getData<FdmNeimaoCrmBusinessApi.Business>();
...
const [Modal, modalApi] = useVbenModal({
  async onConfirm() {
    ...
    // 提交表单
    const data =
      (await formApi.getValues()) as FdmNeimaoCrmBusinessApi.Business;
  ...
  async onOpenChange(isOpen: boolean) {
    ...
    // 加载数据
    const data = modalApi.getData<FdmNeimaoCrmBusinessApi.Business>();
    if (!data || !data.id) {
      return;
    }
    modalApi.lock();
