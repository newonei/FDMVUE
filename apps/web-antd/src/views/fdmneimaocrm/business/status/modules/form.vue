...
const formData = ref<FdmNeimaoCrmBusinessStatusApi.BusinessStatus>();
...
const [Modal, modalApi] = useVbenModal({
  async onConfirm() {
    ...
    // 提交表单
    const data =
      (await formApi.getValues()) as FdmNeimaoCrmBusinessStatusApi.BusinessStatus;
    try {
  ...
  async onOpenChange(isOpen: boolean) {
    ...
    // 加载数据
    const data =
      modalApi.getData<FdmNeimaoCrmBusinessStatusApi.BusinessStatus>();
    modalApi.lock();
    try {
      if (!data || !data.id) {
        formData.value = {
