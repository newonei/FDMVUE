...
| 区域 | 作用 |
| --- | --- |
| 顶部工具栏 | 保存状态与立即保存、导入/导出、撤销/重做、缩放、适配画布、试运行和发布任务 |
...
### 4.6 使用画布 Agent 规划工作流

画布右上角的“画布 Agent”适合把一句自然语言需求转换为**可审阅的画布修改建议**。它不会直接替你保存、
删除、改动或执行任何节点；每一步都需要由有权限的协作者确认。

> 如果面板提示“Agent 当前不可用”，说明管理员尚未启用功能，或没有配置
> `creative.agent.default` 默认路由。请不要以手动改请求、粘贴供应商 Key 或绕过模型中心的方式继续操作。

建议按下面的流程使用：

1. 打开“画布 Agent”，新建或选择一个会话。会话和运行记录会保存在当前项目中，刷新页面后仍可查看。
...
顶部保存状态可以点击，它同时保留“立即保存”功能。管理员启用自动保存后，编辑会在约 800ms 后保存；拖动节点只会在松开鼠标后保存最终位置。保存状态会显示“等待自动保存 / 正在保存 / 已保存时间 / 正在重试 / 离线 / 保存冲突 / 保存失败”。网络断开时画布仅暂存在当前浏览器内存中，恢复网络后会继续尝试保存；刷新或关闭前如果仍有未保存快照，浏览器会提示你确认离开。

发布、试运行、运行节点、应用内容规划和应用画布 Agent 建议前，系统都会先完成当前保存队列。若保存失败、网络离线或发生冲突，后续动作会被拦截，避免基于旧版本发布或执行。
...
使用顶部“导出”会生成 `FdmCreativeWorkflowExport v1` 文件，其中只有工作流结构和必要元信息，不含运行结果、日志、Token、临时/签名 URL、Base64 或本地文件地址。“导入”会先显示节点/连线数量和失效素材引用。导入只能替换当前项目的草稿，必须再次确认；如果文件中的 `assetId` 在当前项目不可读、已过期或属于其他项目，可选择“仅导入结构并清空失效素材引用”，系统不会把它悄悄改为其他项目的私有素材。
