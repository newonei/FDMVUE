...
本教程面向第一次使用 `/fdmcreative/workbench/14` 的用户。你不需要一次学会全部节点：先完成一个最小流程，再按需要查阅后面的节点说明。
...
| 区域 | 作用 |
| --- | --- |
| 顶部工具栏 | 保存状态与“保存草稿”、撤销/重做、缩放、适配画布；选择运行范围后运行；“更多”菜单提供导入/导出工作流和发布版本 |
| 左侧节点库 | 搜索节点；双击或按 `Enter` 加入当前视图，或拖到画布指定位置 |
| 中央画布 | 摆放节点、连接输入输出，并查看节点状态 |
| 右侧工作面板 | 在“参数 / 任务 / 助手”间切换；参数页配置所选节点并查看结果，任务页查看进度、历史和错误，助手页规划画布修改 |
| 画布上方快捷入口 | “导入图片”添加本机图片；“任务”查看运行记录；“AI 助手”打开助手页 |
| 画布上方“查找节点” | 节点较多时按名称、类型或运行状态定位节点；快捷键为 `Ctrl/Cmd + F` |
...
7. 点击右侧参数页的“仅运行此节点”可以独立运行当前节点；也可以在顶部点击“从头运行画布”重新执行整张画布。两种方式都会实际执行生成任务。
8. 结果成功后会显示在参数页的结果区，并归档为项目素材；多次运行的结果可在“结果版本”中查看。
...
- 双击节点库条目，或聚焦条目后按 `Enter`：在当前视图中加入并选中新节点。
- 从节点库拖拽：加入画布指定位置。
- 双击画布空白处，或聚焦画布后按 `Tab`：打开节点搜索，选定后在对应位置创建节点。
- 点击画布上方“导入图片”，或将本机图片文件拖到画布：上传到当前项目并创建图片素材节点；每次最多 20 张，每张不超过 25 MB。
- 单击画布节点：打开右侧“参数”页。
- 按住 `Shift` 点击节点可多选，按住 `Shift` 拖动画布空白处可框选；选中多个节点后可一起移动，并通过浮动工具栏复制或删除。
- 拖动画布空白处移动视图；按住 `Space` 可临时使用抓手。`Ctrl/Cmd + 滚轮` 缩放视图。
...
- `Ctrl/Cmd + D`：立即复制选中的节点及其内部连线。
- `Esc`：关闭节点搜索或取消选择。

画布快捷键需要先聚焦画布；在提示词等输入框中编辑时保留文字编辑操作。批量移动、复制或删除可通过一次撤销恢复。
...
上游也可以是“图片生成”节点：把它的图片输出连接到“参考图生图”的参考图输入，运行时会自动传入上游结果，无需再次从素材库选择。连线卡在出图前显示“生成后自动传入”，出图后显示缩略图和“已自动引用”。“补充素材”只用于添加额外参考图。
...
1. 取消节点选择并收起右侧工作面板，在画布底部输入完整创作需求。
...
也可以选中“AI 内容规划”节点，在顶部将范围设为“仅运行此节点”，点击“预览方案”；确认方案后再应用到画布。“从头运行画布”和“所选及下游”仍会实际执行相应范围的任务。

### 4.6 使用 AI 助手规划工作流

点击画布上方的“AI 助手”，或切换右侧工作面板的“助手”页，可以把一句自然语言需求转换为**可审阅的画布修改建议**。它不会直接替你保存、删除、改动或执行任何节点；每一步都需要由有权限的协作者确认。

> 如果面板提示“Agent 当前不可用”，说明管理员尚未启用功能，或没有配置 `creative.agent.default` 默认路由。请不要以手动改请求、粘贴供应商 Key 或绕过模型中心的方式继续操作。

建议按下面的流程使用：

1. 打开“AI 助手”，新建或选择一个会话。会话和运行记录会保存在当前项目中，刷新页面后仍可查看。
2. 写清目标、素材用途和希望的流程，例如：`以当前商品图为参考，新增提示词、图片生成和成果输出；图片完成后保存到资产库。`
3. 如需指向已有内容，请用输入框下方的 `@当前节点`、`@画布节点`、`@资产`、`@提示词` 选择器添加引用。界面显示的是名称，提交和权限校验使用的是稳定 ID；不要在文本中猜测其他项目的 ID。
4. 从本机拖入或粘贴图片、视频、音频时，系统会先上传为当前项目私有素材，再作为 `@资产` 引用。不要粘贴本地路径、Base64、`blob:` / `data:` 地址或临时签名 URL。
5. 等待“规划完成”后，先在 **CanvasPatch 预览** 中检查新增、修改、连线、断开和删除。删除或断线会以危险色显示，且需要再次确认。
6. 选择 **仅应用到画布**，或在确认建议的运行范围后选择 **应用并运行节点 / 下游 / 全部流程**。后者仍会经过原有工作流执行器、模型路由和运行权限校验；它不是 Agent 的自动执行。

如果在审阅期间有其他编辑者保存了草稿，应用会返回“草稿冲突”，并且不会覆盖对方修改。刷新到服务端最新草稿后，重新提交规划即可。网络断开、关闭面板或刷新页面不会让正在规划的 Run 重新提交模型请求；重新打开后会恢复已持久化的状态和结果。

画布 Agent 的协作权限与画布一致：OWNER / EDITOR 可以提交、取消和应用方案；RUNNER 与 VIEWER 可以查看会话、预览和错误，但不能改动画布；RUNNER 仅能在已有 `RUN` 权限时执行已应用的流程。模型选择保留“自动选择（`creative.agent.default`）”，通常无需手动指定模型；只有管理员已配置多个兼容文本结构化输出模型时才需要切换。
...
完整循环结束后单独运行下游，会读取上游最后一轮的结果；不会把全部轮次自动合并成多张参考图。如果之后又单独生成了该上游，则使用更新的结果。最近循环未全部成功或相关配置已改变时，应先完成或重新运行循环。
...
| 操作 | 做什么 | 不会做什么 |
| --- | --- | --- |
| 保存草稿 | 保存当前节点、连线、位置和配置；手动点击会立即完成保存队列 | 不调用模型，不生成不可变版本 |
| 导出 | 下载不含运行记录、临时链接、凭据和供应商任务的工作流结构文件 | 不复制素材文件或导出其他项目数据 |
| 导入 | 先预检结构和素材引用，确认后替换当前草稿 | 不会静默复制跨项目素材或自动合并两个画布 |
| 从头运行画布 | 每次按依赖顺序重新执行整张画布；有未保存修改时会先保存 | 不直接复用上次生成结果；不等于发布 |
| 仅运行此节点 | 只重新执行所选节点；读取上游现有结果或静态输入配置 | 不自动重跑上游；缺少上游结果时先生成对应上游。“AI 内容规划”在此范围显示“预览方案” |
| 运行所选及下游（范围：所选及下游） | 重新执行所选节点及其下游；范围外输入使用已有结果 | 不自动重跑上游及无关分支 |
| 更多 → 发布版本 | 把当前草稿发布为不可变版本 | 不会自动开始模型执行 |

顶部“保存草稿”按钮和 `Ctrl/Cmd + S` 可立即保存，项目名称旁显示保存状态。管理员启用自动保存后，编辑会在约 800ms 后保存；拖动节点只会在松开鼠标后保存最终位置。保存状态会显示“等待自动保存 / 正在保存 / 已保存时间 / 正在重试 / 离线 / 保存冲突 / 保存失败”。网络断开时画布仅暂存在当前浏览器内存中，恢复网络后会继续尝试保存；刷新或关闭前如果仍有未保存快照，浏览器会提示你确认离开。

发布版本、运行全部、运行节点、应用内容规划和应用 AI 助手建议前，系统都会先完成当前保存队列。若保存失败、网络离线或发生冲突，后续动作会被拦截，避免基于旧版本发布或执行。

任务成功、失败或取消后，运行按钮会自动恢复，无需刷新页面。新一轮运行会重新执行生成任务；“按原参数重试”则继续使用原任务快照。任务进行中请等待结束或取消后再启动下一轮。

多人同时修改时，系统不会自动合并图结构或覆盖服务器内容。冲突窗口会展示服务端草稿版本、最后保存人、保存时间与本地未保存快照；请选择以下其中一项：

- **导出本地副本**：先下载安全的工作流 JSON，适合保留当前工作。
- **加载服务器版本**：明确替换当前画布；若还要保留本地工作，请先导出。
- **保留本地，稍后处理**：停止自动保存，稍后再手工比较和处理。

使用顶部“更多 → 导出工作流”会生成 `FdmCreativeWorkflowExport v1` 文件，其中只有工作流结构和必要元信息，不含运行结果、日志、Token、临时/签名 URL、Base64 或本地文件地址。“更多 → 导入工作流”会先显示节点/连线数量和失效素材引用。导入只能替换当前项目的草稿，必须再次确认；如果文件中的 `assetId` 在当前项目不可读、已过期或属于其他项目，可选择“仅导入结构并清空失效素材引用”，系统不会把它悄悄改为其他项目的私有素材。

### 6.1 查看任务和重试失败节点

点击画布上方“任务”，在右侧“任务”页查看当前进度或切换历史任务，也可以筛选失败节点并定位到画布。查看历史任务不会覆盖当前画布参数。

失败节点的“按原参数重试”使用该次任务保存的原始参数和工作流快照，当前画布的新修改不会带入；确认后还会恢复因失败而跳过或过期的下游节点，并可能产生新的模型调用费用。如果要使用刚修改的参数，请从画布重新运行相应节点或范围。

### 6.2 查看和比较结果版本

选中节点后，在右侧“参数”页的“结果版本”查看历次结果。最新版本和包含已采用结果的版本默认展开，其他版本可点击标题展开。

在两张可用图片上分别点击“加入比较”，再点击“并排比较”；两张图片可以来自不同版本。比较时保留原图比例，支持同步缩放和滚动，查看比较不会改变当前采用的结果。需要更换结果时，另行点击“采用此图”；“固定到画布”可把已有结果作为素材继续使用。
...
当前实现提供强类型节点画布、AI 规划预览、图片/视频生成与处理、顺序时间线、成果归档、任务执行和结果版本记录，并支持跨版本并排比较两张图片。正式分镜板以及多轨视频/音频/字幕时间线仍属于后续规划。本文说明当前实现的操作方式，不代表生产环境已完成部署和业务验收。
