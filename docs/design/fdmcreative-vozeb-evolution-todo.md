...
- [ ] `P3-RESULT-01` 新增节点结果查询服务：按 `projectId + nodeId` 分页返回历史成功 nodeRun、attempt、模型、时间、费用摘要和归档 asset。
- [ ] `P3-RESULT-02` 查询只返回 PUBLISHED、项目可读且未清理的资产；失效历史显示“素材已过期”，不返回断链 URL。
- [ ] `P3-RESULT-03` 扩展 `fdmcreative_node_state`，增加 adopted asset/nodeRun、selectionVersion、选择用户和时间；使用 CAS 更新采用版本。
- [ ] `P3-RESULT-04` “采用版本”前校验 asset 确实由该节点历史成功运行产生，且属于当前租户/项目。
- [ ] `P3-RESULT-05` 当节点配置或上游语义变化后，采用版本跟随 node state 变为 STALE；下游运行前要求重新确认或重新生成，不得静默使用语义不匹配旧结果。
- [ ] `P3-RESULT-06` 明确执行输入优先级：本次执行上游成功输出 > 当前且合法的 adopted asset > 可复用 lastSuccessNodeRun > 不可运行。
- [ ] `P3-RESULT-07` “固定到画布”在本阶段创建新的 `image-input / video-input` 素材节点并引用 assetId，不复制物理文件；P4 完成后再按同一协议扩展 `audio-input`，跨项目始终走现有安全复制逻辑。
- [ ] `P3-RESULT-08` 结果历史分页，不把全部历史塞进工作流 JSON或首屏草稿响应。

### 7.3 快捷工具统一协议

- [ ] `P3-TOOL-01` 定义 `MediaToolDescriptor`：适用素材类型、需要能力、生成的节点类型、默认配置、输入端口、输出定位和是否本地执行。
- [ ] `P3-TOOL-02` 快捷工具不直接修改二进制文件；它生成 CanvasPatch 或受控“派生节点”操作，再由用户确认/自动保存。
- [ ] `P3-TOOL-03` 首批只开放已有执行能力可支撑的工具：图片缩放、图片编辑/变体、视频抽帧、视频裁剪、视频规格统一、转场、视频合成、保存资产库。
- [ ] `P3-TOOL-04` 选中结果时自动使用该 assetId；选中生成节点时使用当前 adopted/latest asset。
- [ ] `P3-TOOL-05` 快捷创建节点使用 `graph-adapter` 的批处理历史与兼容端口规则，一次撤销可撤销整次派生操作。
- [ ] `P3-TOOL-06` 工具执行仍使用现有运行 API；不增加绕过工作流的“直接供应商调用”接口。
- [ ] `P3-TOOL-07` 工具栏按模型目录和 route 能力动态启用；未配置能力显示原因，不显示一个必然失败的可点击按钮。

### 7.4 新媒体能力

- [ ] `P3-NEW-01` 新增本地 `image-crop` 节点：矩形裁剪、边界校验、输出格式、透明通道处理和像素上限。
- [ ] `P3-NEW-02` 新增裁剪编辑器，坐标保存为归一化值或原图像素值并带原图尺寸，避免预览缩放造成偏移。
- [ ] `P3-NEW-03` 评估并实现遮罩资产：遮罩作为私有 IMAGE 资产，节点配置只保存 maskAssetId；遮罩与原图尺寸必须一致或有明确缩放策略。
- [ ] `P3-NEW-04` 只有 FDM AI 目录声明对应能力且 provider 契约完整时才启用局部重绘/扩图；否则完成 UI 禁用和配置说明，不伪造通用支持。
- [ ] `P3-NEW-05` 图片分割优先作为本地节点，限制行列、输出数量、总像素和临时磁盘使用；每个分片归档为独立 asset。
- [ ] `P3-NEW-06` 多角度生成先实现为官方工作流模板（提示词模板 + 图生图 + 集合），不创建无法治理的特殊直连接口。
- [ ] `P3-NEW-07` AI 超分辨率只有存在明确 `IMAGE_UPSCALE` 能力和 route 时再加入枚举/provider；普通 `image-resize` 文案不得冒充 AI 超分。

### 7.5 P3 前端体验

- [ ] `P3-FE-01` 节点或结果选中后显示紧凑媒体工具条；工具不适用时隐藏或带原因禁用。
- [ ] `P3-FE-02` 节点检查器增加结果版本 Tab：缩略图、运行时间、模型、尺寸、状态、采用、固定到画布、保存资产库、删除资格。
- [ ] `P3-FE-03` 多结果调用完整展示全部输出，不只显示第一个；采用状态清晰唯一。
- [ ] `P3-FE-04` 图片查看器支持缩放、原图、下载权限和元数据；视频播放器支持时长、尺寸和基础播放控制。
- [ ] `P3-FE-05` 结果被清理、无权读取或文件缺失时使用明确占位，不让图片 404 撑坏节点布局。
- [ ] `P3-FE-06` 工具创建的派生节点自动放在来源节点右侧合适位置并连线，保持 300 节点画布性能。
- [ ] `P3-FE-07` 自动保存冲突期间禁止创建并执行新的媒体工具分支。

### 7.6 P3 测试与退出门禁

- [ ] `P3-T-01` 结果分页、采用版本归属、CAS、stale、已清理素材和跨项目越权测试。
- [ ] `P3-T-02` 执行输入采用优先级和旧结果复用回归测试。
- [ ] `P3-T-03` 每个快捷工具生成的节点、端口、配置和连线都通过前后端验证器测试。
- [ ] `P3-T-04` 图片裁剪边界、透明通道、超大像素、取消、失败后临时文件清理测试。
- [ ] `P3-T-05` 多结果、采用、固定到画布、撤销和刷新恢复前端测试。
- [ ] `P3-T-E2E-01` 生成四张图片 → 选择第二张为采用版本 → 派生裁剪 → 保存资产库，全链路可追踪。
- [ ] `P3-GATE-01` 所有快捷操作在执行记录、FDM AI Usage（如适用）、资产和血缘中可审计。
- [ ] `P3-GATE-02` 未配置能力不会产生必然失败的调用。
...
- [ ] `P4-DOM-01` 在后端和前端增加端口：`AUDIO_ASSET`、`AUDIO_LIST`；字幕需要时增加独立 `SUBTITLE_TRACK`，不要把字幕伪装成 prompt-text。
- [ ] `P4-DOM-02` 更新 `PortCompatibilityMatrix`：单音频可进入音频集合，其他跨类型转换只能通过明确处理节点。
- [ ] `P4-DOM-03` 增加 `audio-input`，从资产库选择 AUDIO 类型素材。
- [ ] `P4-DOM-04` 增加 `audio-generate`，使用 `FdmAiModality.AUDIO + TEXT_TO_AUDIO`，支持供应商 schema 暴露的语音/音效参数。
- [ ] `P4-DOM-05` 增加 `music-generate`，使用 `FdmAiModality.MUSIC + TEXT_TO_MUSIC`，不与普通音频 route 混用。
- [ ] `P4-DOM-06` 增加 `audio-trim`、`audio-normalize`、`audio-mix`、`audio-extract`、`video-audio-merge` 本地节点。
- [ ] `P4-DOM-07` 音频节点定义明确采样率、声道、格式、起止时间、淡入淡出、音量和输出时长边界。
- [ ] `P4-DOM-08` `audio-mix` 支持有序 AUDIO_LIST，顺序来自明确列表/时间线数据，不来自节点坐标。
- [ ] `P4-DOM-09` `video-audio-merge` 明确原音轨保留、替换、ducking 和最短/最长时长策略。

### 8.3 FDM AI 与资产归档

- [ ] `P4-AI-01` `FdmAiCreativeGateway.route` 和 AI 节点识别支持 AUDIO/MUSIC 节点，不影响现有图片/视频路由。
- [ ] `P4-AI-02` UI 按 modality/capability 过滤音频和音乐模型，未选择模型时使用启用 route 的默认模型。
- [ ] `P4-AI-03` 没有 `creative.audio.generate.default` 或 `creative.music.generate.default` 时返回明确配置错误。
- [ ] `P4-AI-04` `CreativeArtifactService` 能识别供应商音频输出并归档为 `AssetKind.AUDIO`，不直接长期使用供应商 URL。
- [ ] `P4-AI-05` 音频多结果完整归档和展示，每个输出具有稳定 artifactKey，重复 reconcile 不增加重复资产。
- [ ] `P4-AI-06` Provider 不支持某个 voice/music 参数时由模型 schema 隐藏或拒绝，不能把未知字段盲传。

### 8.4 FFmpeg 本地执行

- [ ] `P4-FF-01` 在现有受控 FFmpeg runner 中实现音频命令，不拼接未经转义的用户 shell 字符串。
- [ ] `P4-FF-02` 校验输入 MIME、真实媒体探测、最大时长、最大文件、采样率和声道上限。
- [ ] `P4-FF-03` 长任务不持有数据库事务，继续使用短事务领取、事务外执行、CAS 完成。
- [ ] `P4-FF-04` 取消时终止 FFmpeg 进程，取消获胜后 STAGED 音频不能发布。
- [ ] `P4-FF-05` 临时文件只在受控 work directory，成功或失败都清理；路径不能来自画布配置。
- [ ] `P4-FF-06` 输出先 STAGED，节点成功 CAS 后 PUBLISHED 并建立音频血缘。

### 8.5 音频前端

- [ ] `P4-FE-01` 节点库增加音频分类和完整帮助文本，搜索可找到“配音、音乐、音效、混音、提取音轨”。
- [ ] `P4-FE-02` `NodeInlineEditor` 按节点类型显示音频参数和供应商 schema 参数。
- [ ] `P4-FE-03` 音频素材选择器只展示 AUDIO，支持搜索、播放和时长/格式摘要。
- [ ] `P4-FE-04` 节点和结果版本栈使用可访问的 `<audio controls>` 或统一播放器，不自动播放。
- [ ] `P4-FE-05` 波形不是首版强制项；如实现必须按服务端摘要数据绘制，不能在主线程解码超大文件阻塞画布。
- [ ] `P4-FE-06` 时间线升级前，音频顺序和混音配置仍使用明确表单/列表，不根据画布视觉位置推断。

### 8.6 P4 测试与退出门禁

- [ ] `P4-T-01` 前后端节点目录一致性、端口兼容、必填输入和模型过滤测试。
- [ ] `P4-T-02` AUDIO/MUSIC route、默认模型、无路由、能力不匹配和未知参数测试。
- [ ] `P4-T-03` FFmpeg trim/normalize/mix/extract/merge 的成功、边界、取消、超时、坏文件和清理测试。
- [ ] `P4-T-04` 音频供应商输出归档、幂等、失败不发布和血缘测试。
- [ ] `P4-T-E2E-01` 文本生成配音 → 背景音乐 → 混音 → 合并视频 → 资产库输出。
- [ ] `P4-GATE-01` 音频任务刷新可恢复、可取消、可重试，并能从 `/fdmai/usage` 或本地执行记录追踪。
- [ ] `P4-GATE-02` 图片和视频现有节点全部回归通过。
...
- [ ] `P5A-DB-01` 为 `fdmcreative_project` 增加 `project_type`，默认 `WORKBENCH`，短剧使用 `DRAMA`；现有项目无行为变化。
- [ ] `P5A-DB-02` 新建 `fdmcreative_drama_project`，与 creative project 一对一，保存类型、语言、风格、目标时长、画幅、状态和当前版本引用。
- [ ] `P5A-DB-03` 新建不可变 `fdmcreative_drama_script_revision`，保存 schemaVersion、剧本 JSON、hash、来源 Agent Run/FDM AI invocation、创建人和时间。
- [ ] `P5A-DB-04` 新建 `fdmcreative_drama_entity`，统一保存 `CHARACTER / SCENE / PROP`，使用稳定 entityKey、名称、描述、提示词、锁定状态和 adopted asset。
- [ ] `P5A-DB-05` 角色/场景/道具参考素材只保存 assetId，校验当前项目可读；从其他项目选择时使用现有复制逻辑。
- [ ] `P5A-DB-06` 所有可编辑短剧聚合包含 version/CAS，不能用最后写入静默覆盖团队修改。

#### 剧本结构与 Agent

- [ ] `P5A-DOM-01` 定义 `DramaScript v1` JSON Schema：标题、梗概、主题、角色、场景、道具、场次、台词、旁白、动作和预计时长。
- [ ] `P5A-DOM-02` 剧本生成使用独立 routeKey `creative.drama.script.default` 和 STRUCTURED_OUTPUT。
- [ ] `P5A-DOM-03` 剧本修改先产生 revision preview/diff，用户确认后生成不可变新 revision，不原地覆盖历史版本。
- [ ] `P5A-DOM-04` 提取角色/场景/道具使用稳定 key；新 revision 尽量按 key 匹配，不能按数组位置关联。
- [ ] `P5A-DOM-05` 锁定实体在重新规划时不得被 Agent 静默改写。
- [ ] `P5A-DOM-06` 剧本内容、实体提示词和引用继续使用提示词库/资产库，不另建第二套素材系统。

#### API 与前端

- [ ] `P5A-API-01` 短剧项目创建、分页、详情、更新、归档接口；仍经过 ProjectAccessService。
- [ ] `P5A-API-02` 剧本生成、状态、SSE、预览、确认和 revision 列表接口。
- [ ] `P5A-API-03` 角色/场景/道具分页、编辑、锁定、生成参考图和采用版本接口。
- [ ] `P5A-FE-01` 新增短剧项目列表，工作台项目列表默认不混入 DRAMA，除非明确筛选“全部”。
- [ ] `P5A-FE-02` 短剧详情使用阶段导航，不用超大单页：项目设定、剧本、项目资产、分镜、制作、合成。
- [ ] `P5A-FE-03` 剧本编辑区支持场次结构、台词、旁白和动作编辑；保存走 CAS。
- [ ] `P5A-FE-04` 角色/场景/道具使用紧凑列表/网格，支持资产库选择、图片生成和采用版本。

#### P5A 门禁

- [ ] `P5A-T-01` 剧本 Schema、版本不可变、diff、锁定实体、CAS 和跨项目引用测试。
- [ ] `P5A-T-02` 权限矩阵和刷新恢复测试。
- [ ] `P5A-GATE` 能从一句需求生成并确认结构化剧本，形成可维护角色/场景/道具资产，才进入 P5B。

### 9.3 P5B：分镜和镜头生产

#### 数据模型

- [ ] `P5B-DB-01` 新建 `fdmcreative_drama_shot`：稳定 shotKey、场次号、镜头号、顺序、景别、镜头运动、动作、台词、旁白、时长、连续性组、状态、锁定、version。
- [ ] `P5B-DB-02` shot 保存 adopted image/video/audio assetId，但所有历史结果仍来自 nodeRun/asset lineage，不在 shot 表复制完整输出。
- [ ] `P5B-DB-03` 新建 `fdmcreative_drama_shot_task`：shot、任务类型、executionId、nodeRunId、状态、attempt、错误和采用结果；唯一业务键防重复提交。
- [ ] `P5B-DB-04` 镜头顺序使用明确整数/排序键和 CAS 批量更新，不从卡片坐标推断。

#### 分镜生成与工作流桥接

- [ ] `P5B-DOM-01` 定义 `DramaStoryboard v1` Schema，从确认剧本生成镜头；总时长、单镜时长和镜头数量受配置上限约束。
- [ ] `P5B-DOM-02` 重新生成分镜时按 shotKey 匹配；锁定镜头不被修改，删除/新增必须在 diff 中显示。
- [ ] `P5B-DOM-03` 新建 `DramaWorkflowAssembler`，把单镜图片、视频或批量任务编译成普通强类型 WorkflowDefinition。
- [ ] `P5B-DOM-04` 系统生成的 definition 同样 normalize、validateStructure、validateExecutable 和循环展开。
- [ ] `P5B-DOM-05` 重构执行创建边界，支持受控 `CreativeExecutionLaunchCommand`：来源类型/ID、项目、请求用户、不可变 workflow snapshot、scope；现有工作台 run 也走同一内部创建逻辑。
- [ ] `P5B-DOM-06` `execution_run` 增加 `source_type / source_id` 或等价关联，工作台默认 WORKBENCH；不得破坏现有查询。
- [ ] `P5B-DOM-07` Drama 调用内部 launch 仍校验 RUN 权限、并发槽、租户策略、FDM AI route 和资产归属。
- [ ] `P5B-DOM-08` 一个 shot task 的幂等键确保刷新、双击和 reconcile 不会重复创建模型任务。
- [ ] `P5B-DOM-09` 图片成功后可以生成视频；如果图片采用版本变化，相关视频镜头标记 stale，不能静默保留为当前成品。

#### 前端

- [ ] `P5B-FE-01` 分镜板以镜头卡片展示场次、镜头号、提示词、台词、时长、状态、采用图片/视频。
- [ ] `P5B-FE-02` 支持镜头编辑、锁定、排序、批量选择、生成分镜图、生成视频、取消、失败重试和定位执行详情。
- [ ] `P5B-FE-03` 每个镜头复用 P3 结果版本栈，允许采用版本和固定到资产库。
- [ ] `P5B-FE-04` 提供“打开底层工作流”入口：生成可编辑草稿副本或只读执行快照；不能让手工改动反向破坏 shot 真相。
- [ ] `P5B-FE-05` 列表虚拟化或分页，长剧本不得一次渲染所有高清视频播放器。

#### P5B 门禁

- [ ] `P5B-T-01` shotKey 稳定、锁定、diff、排序 CAS、幂等任务和 stale 传播测试。
- [ ] `P5B-T-02` 系统生成工作流与用户工作流共享执行创建逻辑的回归测试。
- [ ] `P5B-T-E2E-01` 确认剧本 → 生成分镜 → 批量生成图片 → 单镜采用 → 生成视频，刷新后状态完整恢复。
- [ ] `P5B-GATE` 至少完成一个多镜头项目的图片/视频生产闭环，才进入 P5C。

### 9.4 P5C：配音、字幕、时间线和合成

#### 数据模型和时间线

- [ ] `P5C-DB-01` 定义不可变 `DramaTimeline v1`：帧率、画布尺寸、视频轨、对白轨、旁白轨、音乐轨、音效轨、字幕轨。
- [ ] `P5C-DB-02` clip 使用 assetId、startFrame、durationFrames、trimIn/Out、volume、transition，不使用浮点秒作为唯一真相。
- [ ] `P5C-DB-03` 新建 `fdmcreative_drama_composition_revision`：timeline JSON/hash、关联素材版本、executionId、输出 assetId、状态、错误和创建人。
- [ ] `P5C-DB-04` 时间线编辑使用 version/CAS；发布合成时冻结不可变 revision。

#### 配音、字幕和合成

- [ ] `P5C-DOM-01` 根据角色和台词生成配音，声音配置只引用允许的 voice/model 参数，不持久化供应商凭证。
- [ ] `P5C-DOM-02` 旁白、角色对白、背景音乐和音效通过 P4 音频节点执行并进入资产血缘。
- [ ] `P5C-DOM-03` 字幕从确认台词和音频时长生成，保存结构化 cue；导出 SRT/VTT 时生成私有 DOCUMENT 资产。
- [ ] `P5C-DOM-04` 时间线校验素材存在、时长合法、轨道不越界、转场不超过相邻 clip、音量范围和总输出上限。
- [ ] `P5C-DOM-05` `DramaCompositionWorkflowAssembler` 把冻结时间线编译为现有 FFmpeg 节点/受控本地执行工作流。
- [ ] `P5C-DOM-06` 合成结果作为最终资产 `expiresAt=NULL`，建立与所有输入镜头、音频和字幕的血缘。
- [ ] `P5C-DOM-07` 合成失败保留 revision 和错误，可用同一 revision 幂等重试；不能创建重复最终资产。

#### 前端

- [ ] `P5C-FE-01` 首版时间线提供多轨列表和可控拖动，所有修改落入明确 frame 数据。
- [ ] `P5C-FE-02` 提供镜头预览、对白/旁白试听、字幕校对、音乐音量和转场编辑。
- [ ] `P5C-FE-03` 合成前展示缺失素材、预计时长、分辨率、帧率和不可报价状态。
- [ ] `P5C-FE-04` 合成任务进入统一任务面板，可取消、恢复、查看执行和错误。
- [ ] `P5C-FE-05` 完成后支持预览、下载权限、保存资产库和查看血缘。

#### P5C 门禁

- [ ] `P5C-T-01` 时间线 frame 计算、裁剪、转场、音轨、字幕和边界测试。
- [ ] `P5C-T-02` FFmpeg 合成取消、超时、坏素材、临时文件清理和失败不发布测试。
- [ ] `P5C-T-03` 最终资产幂等和完整血缘测试。
- [ ] `P5C-T-E2E-01` 多镜头视频 + 配音 + 音乐 + 字幕 → 合成 → 下载/资产库的完整流程。
- [ ] `P5C-GATE` 短剧 MVP 从需求到最终视频全链路完成，且刷新、失败、重试、权限、Usage、资产和血缘可追踪。
...
- [ ] 当前阶段新增测试通过。
- [ ] `WorkflowDefinitionValidatorTest` 通过。
- [ ] `ProjectAccessServiceTest` 通过。
- [ ] `CreativeExecution*Test` 通过。
- [ ] `CreativeArtifactServiceTest` 通过。
- [ ] `FdmAiCreativeGatewayTest` 通过。
...
- [ ] fdmcreative Vitest 通过。
- [ ] fdmcreative TypeScript 类型检查通过。
- [ ] web-antd 生产构建通过。
- [ ] 没有通过大范围 eslint disable、`any`、`@ts-ignore` 或删除测试规避错误。
